...
static int parse_row(FILE *p_file, sfloat *arr, char *line, int bufsize, int *p_ncol) {
    char c, *p_line = line;
...
void pprint_array(const sfloat *p_arr, const int n, const int m, const char *string) {
    printf("%s\n", string);
    print_array(p_arr, n, m);
}

void print_array(const sfloat *p_arr, const int n, const int m) {
    int i, j;
    for (i = 0; i < n; i++) {
        for (j = 0; j < m; j++)
            printf(NUMFMT, p_arr[i * m + j]);
        printf("\n");
    }
}
...
sfloat norm2(const sfloat *vec, const int n) {
    int i;
    sfloat sum = 0;
    for (i = 0; i < n; i++)
        sum += square(vec[i]);
